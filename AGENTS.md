...
| `jd-api-fetch.mjs` | Bash-callable JD-text fetcher for known-API ATS hosts (Greenhouse/Lever/Ashby/Workday — `liveness-api.mjs`'s `JD_TEXT_API_ATS`), no browser needed; `--classify` reports whether a URL is `api` (headless-safe) or `browser-required` without fetching. Wired into `batch-runner.sh`'s prefetch and its `--extraction api\|browser\|all` filter; `browser-extract.mjs`'s `jd` mode uses the same underlying `fetchJdViaKnownApi()` for the interactive pipeline |
