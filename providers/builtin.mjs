...
// Built In provider — aggregator discovery feed (zero-token, no API key).
//
// Built In is a US tech job aggregator spanning MANY employers, not a single
// tracked company, so it has NO detect() and is reached only via an explicit
// `provider: builtin` entry in portals.yml.
//
// ── Hosts ────────────────────────────────────────────────────────────────
...
// this board offers. Measured on the same four queries, one page each:
//
//   builtin.com          89 rows -> 11 on-policy for a Seattle+US-remote profile (12%)
//   www.builtinseattle   64 rows -> 62 on-policy (97%), of which 24 were US-remote
//
// Only 8 job ids appeared in both. A market host also carries that market's
// remote inventory, so it is not "onsite only".
//
// ── What we parse ────────────────────────────────────────────────────────
...
//      yields nothing, the provider degrades to exactly its pre-enrichment
//      behaviour rather than dropping jobs.
//
...
// Before it, `job.location` and `job.company` were always empty, so the
// blacklist gate, location_filter, posting-age filter and salary_filter were
// all inert here — off-policy rows reached data/pipeline.md and only got culled
// by a human or by an evaluation that costs a full report.
//
...
// rejects real jobs, while an empty one merely reverts to the old behaviour.
// A multi-location card whose tooltip we cannot read therefore yields '' — it
// keeps flowing through exactly as it did before, instead of being dropped on a
// location string ("3 Locations") that names no city.
//
 ...
 * host can now come from config, so this is the only thing standing between a
 ...
 * multi-location card behaves exactly as it did before enrichment, whereas a
 * placeholder like "3 Locations" would be rejected by an allow-list that names
 * cities and remote markers.
 *
